                    ...
                    builder: (context, AsyncSnapshot<QuerySnapshot> snapshot) {
                      if (snapshot.data == null) {
                        ...
                        return RaisedButton(
                          onPressed: () async {
                            if (_character ==
                                SingingCharacter.virtual_account) {
                              if (snapshot.data.documents.isEmpty) {
                                validasiPemayaran(saldoBank, refbank, "BANK");
                                PembayaranBase().virtualSaldoBank(
                                    widget.coffe, widget.totalHarga);
                              } else {
                                var totalsaldoKasir = widget.totalHarga +
                                    snapshot.data.documents[0]['total_saldo'];
                                validasiPemayaran(saldoBank, refbank, "BANK");
                                PembayaranBase().virtualSaldoBankUpdate(
